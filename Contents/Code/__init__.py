...
def FullEpMenu(title):
    ...
    for item in HTML.ElementFromURL(FULLEP_URL).xpath('//div[@class="widget pod tvl-library"]/ul/li'):

        title = item.xpath('./div/h4/a/text()')[0]
...
def VideoBrowse(url, title):
    ...
    for item in HTML.ElementFromURL(url).xpath('//div[@class="crsl-wrap"]/ul/li/@data-video'):
        # To prevent any issues with elements that do not contain the video playlist json, we put the json pull in a try/except
        try:
            video = JSON.ObjectFromString(item)
        except:
            video = None
